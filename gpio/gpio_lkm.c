...
#define BUF_SIZE 512
#define NUM_COM 6 /* number of commands that this driver support */

/* buffer with set of supported commands */
const char * commands[NUM_COM] = {"out", "in", "low", "high", "poweron", "poweroff"};
...
static ssize_t gpio_lkm_write ( struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
    ...
    uint gpios[MAX_GPIO_NUMBER];

    /* get gpio device minor number 
    */
    gpio = iminor(filp->f_path.dentry->d_inode);
    // TODO: init all needed gpios for the device
    ...
    case power_on:
    {
        // TODO: values to corresponding gpios to power on

        //  perform sth like that for every gpio
